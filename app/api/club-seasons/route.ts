...
export async function GET(request: Request) {
  ...
  try {
        ...
        requests: [
          {
            type: "execute",
            stmt: {
              sql: "SELECT DISTINCT season, competition, season_id, tournament_id FROM club_season_aggregates WHERE team_id = ? ORDER BY season DESC",
              args: [{ type: "integer", value: team_id }]
            }
          },
          { type: "close" }
        ]
    ...
    // The Safety Net: Handles both Turso HTTP response formats perfectly
